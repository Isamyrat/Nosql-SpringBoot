...
import com.example.nosql.jwt.AuthEntryPointJwt;
import com.example.nosql.jwt.JwtFilter;
import com.example.nosql.jwt.JwtUtils;
import com.example.nosql.service.impl.UserServiceImpl;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.antMatcher("/api/v2/**").cors().and().csrf().disable()
            .authorizeRequests()
            .antMatchers("/api/v2/user/**").hasRole("ADMIN")
            .and()
